...
function changePerson() {  // 只有执行这个命令，监视器才改变，也就是说监视器监视的是对象地址，而不是对象里面的值
    Object.assign(person, { name: '李四', age: 20 })  // 这里没有修改地址
}
// 监视sum的变化
...
// 如果使用reactive 默认开启深度监视【且无法关闭】，会监视对象内部的属性变化
// 参数1：要监视的对象
...
watch(person,
    (newVal, oldVal) => {
      ...
      // immediate: true 表示立即执行一次回调函数[界面加载第一次的时候也会执行一次回调函数]
)
</script>
